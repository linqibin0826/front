      ...
      <div>
        <article class="c-v-pic-wrap" style="height: 357px;">
          <section class="p-h-video-box" id="videoPlay">
            <img width="100%" height="100%" :src="courseInfo.cover" :alt="courseInfo.title" class="dis c-v-pic">
          </section>
        </article>
                  ...
                  <section class="mt20">
                    <div class="lh-menu-wrap">
                      <menu id="lh-menu" class="lh-menu mt10 mr10">
                        <ul>
                          <!-- 文件目录 -->
                          <li class="lh-menu-stair" v-for="chapter in chapterInfo" :key="chapter.id">
                            <a href="javascript: void(0)" :title="chapter.title" class="current-1">
                              <em class="lh-menu-i-1 icon18 mr10"></em>{{ chapter.title }}
                            </a>
                            <ol class="lh-menu-ol" style="display: block;">
                              <li class="lh-menu-second ml30" v-for="item in chapter.children" :key="item.id">
                                <a :href="'/course/video/' +item.videoSourceId" target="_blank">
                                  <span class="fr">
                                    <i class="free-icon vam mr10">立即观看</i>
                                  </span>
                                  <em class="lh-menu-i-2 icon16 mr5">&nbsp;</em>{{ item.title }}
                                </a>
                              </li>
                            </ol>
                          </li>
...
export default {
  ...
  mounted() {
    this.getFrontCourseInfoById(this.courseId)
  },
  methods: {
    getFrontCourseInfoById(id) {
      course.getFrontCourseInfoById(id).then(response => {
        this.courseInfo = response.data.data.data.courseInfo
        this.chapterInfo = response.data.data.data.chapterInfo
      })
    },
    createOrder() {
